...
export default function Profile() {
  ...
  const handleInputChange = <K extends keyof AttendeeData>(
    field: K,
  ...
  const handleResumeUpdate = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (file) {
      setAttendeeData((prev) => ({ ...prev, resume: file.name }));
